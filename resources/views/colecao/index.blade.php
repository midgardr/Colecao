...
@section('conteudo')
    ...
    <table class="table table-striped table-bordered table-hover">
        <thead class="table-primary">
            <tr>
                <td colspan="4">
                    <form action="{{ route('colecoes', isset(request()->route()->parameters['eLixeira'])) }}" method="get" enctype="application/x-www-form-urlencoded">
                        <div class="input-group mb-3">
                            ...
                            <span class="input-group-text">
                                ...
                                <a href="{{ route('colecoes', isset(request()->route()->parameters['eLixeira'])) }}" class="btn"><i class="bi bi-x-square-fill"></i></a>
                                @if(isset(request()->route()->parameters['eLixeira']))
                                    <a href="{{ route('colecoes') }}" class="btn btn-success"><i class="bi bi-table"></i></a>
                                @else
                                    <a href="{{ route('colecoes', true) }}" class="btn btn-danger"><i class="bi bi-trash"></i></a>
                                @endif
                            </span>
                        </div>
                    </form>
        ...
        <tbody>
            @forelse($colecoes as $colecao)
                <tr>
                    <td width="30%">{{ $colecao->nome }}</td>
                    <td width="50%">{{ Str::limit($colecao->descricao, 30, '...', true) }}</td>
                    <td width="10%">{{ isset(request()->route()->parameters['eLixeira']) ? $colecao->deleted_at : $colecao->created_at }}</td>
                    <td width="10%">
                        @if(isset(request()->route()->parameters['eLixeira']))
                            <a href="{{ route('colecoes.restore', $colecao->id) }}" class="btn btn-success"><i class="bi bi-recycle"></i> Restaurar</a>
                        @else
                            <a href="{{ route('colecoes.edit', $colecao->id) }}" class="btn btn-success"><i class="bi bi-pencil-square"></i> Editar</a>
                            <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#modelColecaoDelete" data-bs-colecao-id="{{ $colecao->id }}" data-bs-colecao-nome="{{ $colecao->nome }}"><i class="bi bi-trash"></i> Excluir</button>
                        @endif
                    </td>
                </tr>
            @empty
